...
For each incoming audio chunk it:
  ...
  5. validates each new claim against the KB (phase2_validator.validate_claim)
  6. returns alert objects (schema = REQUIREMENTS_REALTIME.md §7.1)
...
class LiveSession:
    ...
    def _validate(self, claims):
        ...
        for claim in claims:
            try:
                result = validate_claim(claim, self.kb)
            except Exception:
    ...
    def _evidence(self, claim, result):
        """Attach source snippets from the KB for the sources the validator cited."""
        evidence = []
        try:
            q = self.kb.query(query_texts=[claim], n_results=3)
            docs = (q.get("documents") or [[]])[0]
            metas = (q.get("metadatas") or [[]])[0]
            cited = set((result.get("supporting_sources") or []) +
                        (result.get("conflicting_sources") or []))
            for doc, meta in zip(docs, metas):
                meta = meta or {}
                src = meta.get("source", "unknown")
                if cited and src not in cited:
                    continue
                evidence.append({
                    "source": src,
                    "snippet": (doc or "")[:200],
                    "freshness": meta.get("date") or meta.get("updated") or meta.get("freshness"),
                })
        except Exception:
            pass
        return evidence
